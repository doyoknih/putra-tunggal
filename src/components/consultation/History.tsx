...
const HistoryChat: FC<{
  ...
  const onlineRoom = rooms.filter((room) =>
    moment(room.expiredDate).isAfter(moment())
  );
  const expiredRoom = rooms.filter((room) =>
    moment(room.expiredDate).isBefore(moment())
  );
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-5 border-b-2 border-black/30 pb-5">
        ...
        {onlineRoom.length == 0 ? (
          ...
          onlineRoom.map((item, index) => {
            const user = item.RoomUser.find(
              (user) => user.user.role.role === "Dokter"
            );
            return (
              <div
                key={index}
                className="flex items-start gap-2 md:gap-5 cursor-pointer"
      ...
      <div className="flex flex-col gap-5 overflow-hidden overflow-y-scroll h-full">
        ...
        {expiredRoom.length == 0 ? (
          <p>No Expired Chat</p>
        ) : (
          expiredRoom.map((item, index) => {
            const user = item.RoomUser.find(
              (user) => user.user.role.role === "Dokter"
            );
            return (
